...
class GRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        self.use_em_loss = args.use_em_loss
            ...
            if self.vllm_mode == "server" and self.accelerator.is_main_process:
                ...
                self.llm = LLM(
                    ...
                    * self.args.gradient_accumulation_steps,
                    max_model_len=self.max_prompt_length + self.max_completion_length if self.max_prompt_length is not None else self.max_completion_length,
    ...
    def _calculate_rewards(self, inputs, prompts, completions, completion_ids_list):
        ...
        # Skip reward calculation when using entropy-based rewards
        #if self.use_em_loss:
    ...
    def _compute_loss(self, model, inputs):
        ...
        # Compute the loss
        if self.use_em_loss:
            # Use entropy-based rewards: negative entropy summed over tokens in each sequence
            sequence_entropy_rewards = -(entropies * completion_mask).sum(dim=1)  # Shape: [batch_size]
            advantages = sequence_entropy_rewards
        else:
            # Use original advantages from inputs
            advantages = inputs["advantages"]
        ...
        avg_entropy = entropies.nanmean().item()

        # Optionally: max/min entropy values across batch (e.g., how confident is the model at worst/best?)
        max_entropy = nanmax(entropies).item()
        min_entropy = nanmin(entropies).item()

        # Log to metrics
        self._metrics[mode]["entropy/avg_tokenwise"].append(avg_entropy)
        self._metrics[mode]["entropy/max"].append(max_entropy)
        self._metrics[mode]["entropy/min"].append(min_entropy)
        return loss
